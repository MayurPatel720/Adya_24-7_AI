...
export async function POST(request: NextRequest) {
  try {
    ...
    const aiResponse = await generateAIResponse(message, from);

    if (aiResponse) {
      await new Promise(resolve => setTimeout(resolve, AI_RESPONSE_DELAY));
      const success = await sendTextMessage(from, aiResponse);
      return NextResponse.json({ success, from });
    }
...
export async function GET() {
  return NextResponse.json({ status: 'ok', messages: 'Bot active', service: 'cloud_api' });
}
